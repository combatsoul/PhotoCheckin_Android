...
public class LoginForm extends Activity implements View.OnClickListener {
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(
						2);
				nameValuePairs.add(new BasicNameValuePair("textUsername",
						input_username.getText().toString()));
				nameValuePairs.add(new BasicNameValuePair("textPassword",
						...
						for (String word2 : sentence) {
							if (word2.equals(notblock)) {
								if (value.equals("0")) {
									...
									// when can't login it will close page
									// loginform last
							...
							if (word2.equals(block)) {
								...
								// when can't login it will close page loginform
								// last
								finish();
								Intent goMain = new Intent(
										getApplicationContext(),
						...
						for (String word2 : sentence) {
							if (word2.equals(notblock)) {
								if (value.equals("0")) {
									...
									// when login done sent value usename to
									// wallpage
									Intent goMain = new Intent(
											getApplicationContext(),
											WallPage.class);
									goMain.putExtra("username", input_username
											.getText().toString());
									startActivity(goMain);
	...
	public void chkinternet(View v) {
		...
		if (isConnectInternet()) {
			Toast.makeText(getApplication(), "Connected Internet",
					Toast.LENGTH_LONG).show();
		} else {
			Toast.makeText(getApplication(), "Connected Internet Error",
					Toast.LENGTH_LONG).show();
		}
